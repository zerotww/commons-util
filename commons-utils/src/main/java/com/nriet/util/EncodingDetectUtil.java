...
/**
 * 
 * @ClassName: EncodingDetectUtil
 * @Description: 资料文件格式编码的转换
 * @author liqingbo
 * @date 2014年9月22日 上午10:52:42
 *
 */
public class EncodingDetectUtil {
	public static void main(String[] args) {
		String file = "C:/Documents and Settings/Administrator/climate/xun3/a.txt";
	...
	public static String getJavaEncode(String filePath) {
		...
		if (filePath.startsWith("smb")) {
			fileCode = BytesEncodingDetect.javaname[s.detectEncoding(filePath)];
		} else {
	 ...
	 * @throws
	 */
	public static String getJavaEncodeSmb(String filePath) {
		BytesEncodingDetect s = new BytesEncodingDetect();
		String fileCode = BytesEncodingDetect.javaname[s.detectEncoding(filePath)];
		return fileCode;
	}
...
/**
 * 
 * @ClassName: BytesEncodingDetect
 * @Description: 内部类的格式转换
 * @author JSCollect
 * @date 2014年9月22日 上午10:54:26
 *
 */
class BytesEncodingDetect extends Encoding {
	...
	public int detectEncoding(String path) {

		byte[] buff = null;
		SmbFileInputStream chinesefile = null;

		try {
			SmbFile smbfile = new SmbFile(path);
			chinesefile = new SmbFileInputStream(smbfile);
			;
			int bufferSize = smbfile.getContentLength();
			buff = new byte[bufferSize];
			chinesefile.read(buff);
